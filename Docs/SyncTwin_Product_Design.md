...
6. 如果某一侧目录相对共同基线突然被清空，或在大目录场景下异常接近清空，系统必须优先保护仍存在的文件，不把这批删除直接扩散到另一台电脑。
...
7. 在保留增量扫描性能优势的前提下，系统需要识别“目录疑似被误清空”的场景，并自动切换到安全回补策略。
...
4. 如果某一侧出现“共同基线非空，但当前结果变成空目录”或“大目录下大比例删除后仅剩极少量路径”的信号，则把这一侧标记为疑似误清空侧；对于该侧报出的删除，只要另一侧该路径仍然存在，就优先执行回补，不把删除传播出去。
